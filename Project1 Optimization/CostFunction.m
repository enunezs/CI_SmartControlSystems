...
%% Calculate error
%C=sqrt( sum(abs(response-ref_func).^2) ) / samples;     %Error Square
%C=sum(abs(response-ref_func))/samples;  %Linear error


C= sqrt(sum(abs(response([1:50000 60000:end])-ref_func([1:50000 60000:end])).^2))/samples;  %Error except on  5
%Escluir rango 4.8-5.2

%Output calculated cost for parameters
